...
本包为 Web 客户端侧栏和空会话首屏提供 ThunderUni 标志与本地化名称。浏览器插件仅在 `DSH_CLIENT_BUILD_PROFILE` 为 `thunderuni` 时注册 ThunderUni 填充；其他 profile 保留自己的填充或外壳回退。本包只负责呈现，不改变 agent 请求、权限、模型、会话或网络行为。
...
本包拥有 `thunderuni` locale 命名空间，并提供英文与简体中文词典。侧栏本地化名称接收标准类型化 `t` seat；几何标志使用提供的 `thunderuni-ai-logo.svg` 素材，并以内嵌 data URI 形式打入 Web bundle，不依赖运行时文件路径。
...
浏览器半部等待两个侧栏品牌声明后再注册一组填充。同一注册组还占据 `conversation.hero.brand.mark`，因此空会话首屏会使用提供的 SVG，而不是动画鱼形回退。声明收回时三个填充一起撤回，插件销毁时 locale 词典与活动 slot 注册也一并移除。Host 半部为空，因为本包没有服务端行为。
...
- **只替换客户端身份**——本包会替换侧栏与空会话首屏 artwork，但不改变服务端产品文案。
