...
The composer context meter reads the latest request capacity from the `contextPressure` projection. A live model-capacity update can replace that denominator immediately for the matching selected route; the next model request records the same capacity durably, while a reloaded Session keeps its last recorded capacity until another request resolves the route.
