...
`dsh-client-ui-settings-models` 是 dsh Web 客户端的 Models 设置页面：用户可以配置 API 密钥（以只写方式存入 profile 的凭据引用之下）、编辑每个提供方的模型列表，并手工声明自定义 pi-ai 路由；页面以提供方行展示，一次只展开一张编辑卡片。该页面把提供方目录、设置文档与凭据描述合并为一个共享快照，因此行的状态在三个方面始终一致。部署启用首次运行引导时，它还会带用户走两个有序弹窗——版本化内测声明，以及按条件显示的官方 DeepSeek 凭据步骤。
...
本插件接受 `onboarding` 配置项。部署自有的 Web profile 可以将其设为 `false`，从而省略两个首次运行弹窗（包括内测声明和官方 DeepSeek 凭据步骤），同时保留 Models 设置页及其提供方编辑器。
