...
export function Sub2apiSection(props: Sub2apiSectionProps): ReactNode {
  const { actions, subscribe, t } = props
  ...
  const submitAuth = (event: React.FormEvent<HTMLFormElement>): void => {
    ...
    void run(mode === 'login' ? 'login' : 'register', () => mode === 'login'
      ? actions.login(input)
      : actions.register({ ...input, ...(captcha.trim() === '' ? {} : { captcha: captcha.trim() }) }))
  ...
  if (!loaded) return <div className={css.section}><p className={css.status}>{t('loading')}</p></div>
    ...
    return (
      <section className={css.section}>
        ...
        <div className={css.tabs} role="tablist" aria-label={t('title')}>
          ...
          <button type="button" role="tab" aria-selected={mode === 'register'} className={mode === 'register' ? css.tabActive : css.tab} onClick={() => { setMode('register'); setFailure(undefined) }}>{t('register')}</button>
        </div>
  ...
  return (
    <section className={css.section}>
      ...
      <div className={css.cards}>
        <Metric label={t('balance')} value={formatBalance(state.usage)} stale={state.usage?.stale === true} staleLabel={t('stale')} />
      ...
      <div className={css.actions}>
        ...
        <button className={css.secondaryButton} type="button" disabled={busy !== undefined} onClick={openRecharge}>{t('recharge')}</button>
        <button className={css.secondaryButton} type="button" disabled={busy !== undefined} onClick={refreshAll}>{t('refreshAfterRecharge')}</button>
      </div>
...
function formatBalance(usage: Sub2apiUsageSnapshot | undefined): string {
  if (usage?.balance === undefined) return '—'
  return `${usage.currency ?? ''} ${usage.balance}`.trim()
}
