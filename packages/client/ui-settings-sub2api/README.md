...
`dsh-client-ui-settings-sub2api` adds the Sub2API account section to the dsh web Settings surface. It submits login, registration, and second-factor forms through the `sub2api` Remote namespace, renders only the Host's account, balance, usage, model, freshness, and stable-error projections, and opens a validated recharge URL without storing or displaying any credential value.
...
The signed-out view offers login and registration. A server-declared second-factor challenge switches to a short-lived code form. The authenticated view shows the safe account identity, balance, usage, model count, stale state, refresh actions, and the validated recharge action. Passwords and one-time codes are cleared after submission; no token, API Key, raw response, or authorization header is rendered.
...
The section uses the existing `settings.section` slot and receives its actions from a small client-side wrapper over `ctx.remote.sub2api`. Host state changes arrive through the forwarded `sub2api/state-changed` event, so account changes, sign-out, stale data, and key recovery update the page without polling.

The browser treats Remote failures as stable error codes and selects localized copy from the package dictionary. It does not inspect or reconstruct Remote error bodies. The recharge button accepts only the URL returned by the Host after the profile's scheme, origin, and redirect policy have approved it.
...
- **No credential ownership** — the browser never reads, stores, or deletes the Host-managed access token, refresh token, or API Key secret.
