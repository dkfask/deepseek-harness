...
The Desktop user's runtime profile controls installed plugins, not the Web brand. `prepare:dsh` does not rebuild the Web frontend, and an already prepared or packaged application does not change when the workspace is edited; rebuild the target to change its embedded client artifacts. An `official` package keeps the `DeepSeek Harness` shell identity. A `thunderuni` package uses the `ThunderUni` shell identity and carries an immutable runtime-default resource that enables the local Sub2API integration at `http://127.0.0.1:8090`; launch-time environment values still take precedence. ThunderUni stores the Sub2API refresh token in the local credential store, refreshes the session during Host startup, and never stores the account password; signing out removes that record.
