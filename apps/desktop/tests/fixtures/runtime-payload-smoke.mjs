...
/** Verify the current native flock entry remains lazy on Windows. */
function checkNativeSystem() {
  const system = requireRuntime('@deepseek-ai/node-addon-system/flock')
  assert.equal(typeof system.tryLockExclusive, 'function')
}
...
process.once('beforeExit', () => {
  console.log(JSON.stringify({ node: process.versions.node, platform: process.platform, arch: process.arch,
    nativeSystem: true, koffi: true, sharp: true, html: true, pty: true }))
})
